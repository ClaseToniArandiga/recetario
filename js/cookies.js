...
function configurarBotonesCookies() {
    ...
    if (btnGuardarPreferencias) {
        btnGuardarPreferencias.addEventListener('click', function (e) {
            ...
            const experiences = document.querySelector('input[name="experiences"]:checked')?.value;
            const advertising = document.querySelector('input[name="advertising"]:checked')?.value;
            const profileAdvertising = document.querySelector('input[name="profile-advertising"]:checked')?.value;

            // Validar que todos los radio buttons estén seleccionados
            if (!performance || !experiences || !advertising || !profileAdvertising) {
                console.warn('[COOKIES.JS] Debe seleccionar una opción para cada sección');
                alert('Por favor, selecciona una opción para cada sección de cookies.');
                return;
            }

            // Si todos están seleccionados, guardar preferencias
            const preferencias = {
